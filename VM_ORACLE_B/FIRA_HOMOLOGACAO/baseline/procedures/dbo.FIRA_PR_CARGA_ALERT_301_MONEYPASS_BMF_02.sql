CREATE PROCEDURE [dbo].[FIRA_PR_CARGA_ALERT_301_MONEYPASS_BMF_02] @PREGAO SMALLDATETIME, @AUX INT
--WITH ENCRYPTION	
AS

--2026-02-25 Gobbo e Guimo
-- coluna preço transformamos volume / quantidade. linhas 145 e 160. 1280 e 1264
--04/03/26 linha 391 e 1452. mudou de '-' para '+' (Heitor e Gobbo)

--2026-04-01 Gobbo e Guimo
-- Retirar abs das colunas vl totneg compra e venda. (mês e 6 meses)
-- Ajuste no calculo da concentração(mês e 6 meses)


--2026-04-01 Gobbo e Guimo
--Ajuste no join da concentração final 6 meses onde faltava o 
--join com o campo ano mes e tabela ao inves da de 6 meses estava olhando para a do mes
...
***********************************/
IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[ST_ALERT_MONEYPASS_BMF_02_PADRAO]') AND type in (N'U'))
CREATE TABLE [dbo].[ST_ALERT_MONEYPASS_BMF_02_PADRAO](
	[DATA] [date] NULL,
	[CD_CLIENTE] [int] NULL,
...
EXEC dbo.usp_sync_table_schema_add_alter_2012
  @src_schema='dbo', @src_table='ST_ALERT_MONEYPASS_BMF_02_PADRAO',
  @dst_schema='dbo', @dst_table='ST_ALERT_MONEYPASS_BMF_02',
  @execute=1,
  @allow_drop=1;
...
*/
	...
	SELECT DT_PERIODO	  AS data
		 , CD_CLIENTE	  AS cod_cliente
		 , HH_NEGOCIO	  AS hr_do_negocio
		 , NR_NEGOCIO	  AS nr_negocio
		 , CD_PAPEL		  AS ativo
		 , CD_NATOPE	  AS operacao
		 , QT_NEGOCIO	  AS qtd_negocios
		 , VL_NEGOCIO	  AS preco
		 , VL_TOTNEG	  AS volume
		 , CD_CONTRAPARTE AS cod_contraparte 
	  INTO #base_util
	  FROM ST_MESA_BVSP
	 WHERE DT_PERIODO BETWEEN @INICIO AND @FIM;*/

 ---------------------------------------------------------------- MES INICIO
 ---------------------------------------------------------------------------
 DROP TABLE IF EXISTS #DIRETAS;
		SELECT DT_NEGOCIO
			 , CD_CLIENTE_COMPRA
			 , CD_CLIENTE_VENDA
			 , NR_NEGOCIO
			 ...
			 , VL_NEGOCIO_COMPRA
			 , VL_NEGOCIO_VENDA
			 , VL_TOTNEG_COMPRA AS VL_TOTNEG_COMPRA
			 , VL_TOTNEG_VENDA  AS VL_TOTNEG_VENDA
			 --, CD_OPERADOR_COMPRA
			 --, CD_OPERADOR_VENDA
			 , HH_COMPRA
			 , HH_VENDA
		  INTO #DIRETAS
		  FROM ST_DIRETAS_BMF 
		 WHERE DT_NEGOCIO BETWEEN @INICIO AND @FIM
		   AND CD_CLIENTE_COMPRA <> CD_CLIENTE_VENDA;

 DROP TABLE IF EXISTS #base_util;		 
		SELECT DT_NEGOCIO							 AS data
			 , CD_CLIENTE_COMPRA					 AS cod_cliente
			 , HH_COMPRA							 AS hr_do_negocio
			 , nr_negocio							 AS nr_negocio
			 , CD_NEGOCIO							 AS ativo --papel
			 , 'C'									 AS operacao
			 , QT_NEGOCIO_COMPRA					 AS qtd_negocios
			 , VL_TOTNEG_COMPRA	/ QT_NEGOCIO_COMPRA  AS preco
			 , VL_TOTNEG_COMPRA						 AS volume
			 , CD_CLIENTE_VENDA						 AS cod_contraparte
		  INTO #base_util 
		  FROM #DIRETAS--select * from #base_util

		 UNION ALL

		SELECT DT_NEGOCIO							AS data
			 , CD_CLIENTE_venda						AS cod_cliente
			 , HH_COMPRA							AS hr_do_negocio
			 , NR_NEGOCIO							AS nr_negocio
			 , CD_NEGOCIO							AS ativo--papel
			 , 'V'									AS operacao
			 , QT_NEGOCIO_VENDA						AS qtd_negocios
			 , VL_TOTNEG_VENDA	/ QT_NEGOCIO_VENDA  AS preco
			 , VL_TOTNEG_VENDA						AS volume
			 , CD_CLIENTE_COMPRA					AS cod_contraparte
		  FROM #DIRETAS;
...
DROP TABLE IF EXISTS #base;
    ...
    SELECT
        ...
        operacao,
        STRING_AGG(CAST(nr_negocio AS varchar(MAX)), ',') AS numeros_negocio,
        SUM(qtd_negocios) AS qtd_total,
        SUM(CAST(preco AS decimal(18,6)) * CAST(qtd_negocios AS decimal(18,0)))
            AS resultado_total
    into #base			
    FROM #clientes_compra_venda
    GROUP BY cod_cliente, cod_contraparte, CAST([data] AS date), operacao
...
DROP TABLE IF EXISTS #nums;
    SELECT
        ...
        t.cod_contraparte,
        t.[data],
        STRING_AGG(CAST(t.token AS VARCHAR(MAX)), ',') AS numeros_negocio
		into #nums
    FROM (
    ...
    GROUP BY t.cod_cliente, t.cod_contraparte, t.[data]
--),
...
DROP TABLE IF EXISTS #ativos_dedup;
    SELECT
        ...
        a.cod_contraparte,
        a.[data],
        STRING_AGG(CAST(a.ativos AS VARCHAR(MAX)), ',') AS ativos
		into #ativos_dedup
    FROM (
        SELECT DISTINCT
            cod_cliente,
    ...
    GROUP BY a.cod_cliente, a.cod_contraparte, a.[data]
...
*/
DROP TABLE IF EXISTS #concentracao; --select * from #concentracao where concentracao_contraparte > '0'
...
WITH dias AS (
  SELECT
    ...
    MAX(d.daytrade)                                      AS dia_tem_daytrade
  FROM #daytrade_df AS d
  GROUP BY d.cod_cliente, d.cod_contraparte, CAST(d.data AS date)
),
ativos_concat AS (
  SELECT
    t.cod_cliente                                        AS cod_cliente,
    t.cod_contraparte                                    AS cod_contraparte,
    STUFF((
      SELECT ',' + x.ativo
      FROM (
  ...
  FROM (
    SELECT DISTINCT d.cod_cliente, d.cod_contraparte
    FROM #daytrade_df AS d
    WHERE d.daytrade = 1
  ) AS t
)
SELECT
  ...
  d.cod_contraparte                                      AS cod_contraparte,
  COUNT(*)                                               AS total_dias,
  SUM(d.dia_tem_daytrade)                                AS dias_daytrade,
  CAST(SUM(d.dia_tem_daytrade) AS decimal(38,6)) / NULLIF(COUNT(*),0) AS proporcao_daytrade,
...
GROUP BY d.cod_cliente, d.cod_contraparte, a.ativos_daytrade;
...
WITH ativos_bal_concat AS (
  SELECT
    b.cod_cliente                                         AS cod_cliente,
    b.cod_contraparte                                     AS cod_contraparte,
    STUFF((
      SELECT ',' + y.ativo
      FROM (
        SELECT DISTINCT b2.ativo
        FROM #balanceamento_ativo AS b2
        WHERE b2.cod_cliente = b.cod_cliente
          AND b2.cod_contraparte = b.cod_contraparte
      ) AS y
      ORDER BY y.ativo
      FOR XML PATH(''), TYPE
    ).value('.','nvarchar(max)'),1,1,'')                  AS ativos_balanceados
  FROM (
    SELECT DISTINCT cod_cliente, cod_contraparte
    FROM #balanceamento_ativo
  ) AS b
)
SELECT
  b.cod_cliente                                           AS cod_cliente,
  b.cod_contraparte                                       AS cod_contraparte,
  AVG(b.balanceamento_ativo)                              AS balanceamento,
  ISNULL(ab.ativos_balanceados,'')                        AS ativos_balanceados
INTO #balanceamento
FROM #balanceamento_ativo AS b
LEFT JOIN ativos_bal_concat AS ab
  ON ab.cod_cliente = b.cod_cliente
 AND ab.cod_contraparte = b.cod_contraparte
GROUP BY b.cod_cliente, b.cod_contraparte, ab.ativos_balanceados;
...
GROUP BY i.cod_cliente, i.cod_contraparte, u.ativo;
 ...
 ------------------------------------------------------------6 MESES  INICIO
 ---------------------------------------------------------------------------
 DROP TABLE IF EXISTS #DIRETAS_6M;
		SELECT DT_NEGOCIO
			 , CD_CLIENTE_COMPRA
			 , CD_CLIENTE_VENDA
			 , NR_NEGOCIO
		  ...
		  INTO #DIRETAS_6M
		  FROM ST_DIRETAS_BMF 
		 WHERE DT_NEGOCIO BETWEEN @INICIO_6M AND @FIM_6M
		   AND CD_CLIENTE_COMPRA <> CD_CLIENTE_VENDA;
 ...
 DROP TABLE IF EXISTS #base_util_6M;		 
		SELECT DT_NEGOCIO	      AS data
			 ...
			 , VL_TOTNEG_COMPRA   AS volume
			 , CD_CLIENTE_VENDA   AS cod_contraparte
			 , CONVERT(CHAR(6),DT_NEGOCIO, 112) AS cd_anomes
		  INTO #base_util_6M 
		  FROM #DIRETAS_6M
...
DROP TABLE IF EXISTS #base_6M;  
    ...
    SELECT
        ...
        SUM(CAST(preco AS decimal(18,6)) * CAST(qtd_negocios AS decimal(18,0)))
            AS resultado_total
     into #base_6M
    FROM #clientes_compra_venda_6M
    GROUP BY cod_cliente, cod_contraparte, CAST([data] AS date), operacao,cd_anomes
--)
...
nums_6M AS (
    SELECT
        ...
        STRING_AGG(CAST(t.token AS VARCHAR(MAX)), ',') AS numeros_negocio
    FROM (
        SELECT DISTINCT cod_cliente, cod_contraparte, [data], token,cd_anomes
        FROM tokens_6M
        WHERE token <> ''
    ) AS t
    GROUP BY t.cod_cliente, t.cod_contraparte, t.[data],t.cd_anomes
),
ativos_dedup_6M AS (
    SELECT
        a.cod_cliente,
    ...
    GROUP BY a.cod_cliente, a.cod_contraparte, a.[data],cd_anomes
)
SELECT
    b.cod_cliente,
    b.cod_contraparte,
...
LEFT JOIN #concentracao_total_6M AS t
  ON t.cod_cliente = c.cod_cliente
and t.cd_anomes = c.cd_anomes; --ajuste
...
DROP TABLE IF EXISTS #dias_6M;
  ...
  GROUP BY d.cod_cliente, d.cod_contraparte, CAST(d.data AS date),cd_anomes
--),

DROP TABLE IF EXISTS #ativos_concat_6M;
  SELECT
    t.cod_cliente                                        AS cod_cliente,
    t.cod_contraparte                                    AS cod_contraparte,cd_anomes,
    STUFF((
      SELECT ',' + x.ativo
      FROM (
        SELECT DISTINCT d2.ativo
        ...
        WHERE d2.cod_cliente = t.cod_cliente
          AND d2.cod_contraparte = t.cod_contraparte
		  and d2.cd_anomes = t.cd_anomes
          AND d2.daytrade = 1
      ) AS x
      ORDER BY x.ativo
      FOR XML PATH(''), TYPE
    ).value('.','nvarchar(max)'),1,1,'')                 AS ativos_daytrade
	into  #ativos_concat_6M
  FROM (
    SELECT DISTINCT d.cod_cliente, d.cod_contraparte , d.cd_anomes
    FROM #daytrade_df_6M AS d
    WHERE d.daytrade = 1
  ) AS t
--)
...
LEFT JOIN #ativos_concat_6M AS a
  ON a.cod_cliente = d.cod_cliente
 AND a.cod_contraparte = d.cod_contraparte
GROUP BY d.cod_cliente, d.cod_contraparte, a.ativos_daytrade, d.cd_anomes;
...
DROP TABLE IF EXISTS #ativos_bal_concat_6M;
  ...
  SELECT
    ...
    STUFF((
      ...
      FROM (
        SELECT DISTINCT b2.ativo
        FROM #balanceamento_ativo_6M AS b2
        WHERE b2.cod_cliente = b.cod_cliente
          AND b2.cod_contraparte = b.cod_contraparte
		  and b2.cd_anomes = b.cd_anomes
      ) AS y
      ORDER BY y.ativo
      FOR XML PATH(''), TYPE
    ).value('.','nvarchar(max)'),1,1,'')                  AS ativos_balanceados
	into #ativos_bal_concat_6M
  FROM (
    SELECT DISTINCT cod_cliente, cod_contraparte , cd_anomes
    FROM #balanceamento_ativo_6M
  ) AS b
--)
SELECT
  b.cod_cliente                                           AS cod_cliente,
...
LEFT JOIN #ativos_bal_concat_6M AS ab
  ON ab.cod_cliente = b.cod_cliente
 AND ab.cod_contraparte = b.cod_contraparte
 and ab.cd_anomes = b.cd_anomes
GROUP BY b.cod_cliente, b.cod_contraparte, ab.ativos_balanceados , b.cd_anomes;
...
GROUP BY i.cod_cliente, i.cod_contraparte, u.ativo,i.cd_anomes;
 ...
 DROP TABLE IF EXISTS #MoneyPass_6M_metrics_B;
  ...
  DROP TABLE IF EXISTS #MoneyPass_6M_metrics			   
    ...
    DELETE FROM ST_ALERT_MONEYPASS_BMF_02 WHERE DATA = CAST(@PREGAO-@AUX AS DATE)            
             
    INSERT INTO ST_ALERT_MONEYPASS_BMF_02 (DATA,CD_CLIENTE,CD_CONTRAPARTE,NR_NEGOCIOS,PAPEIS,QTD_TOTAL,RESULTADO,RESULTADO_MEDIA_6M,RESULTADO_DESVIO_6M,CONCENTRACAO,CONCENTRACAO_MEDIA_6M,CONCENTRACAO_DESVIO_6M,IND_ACERTO,IND_ACERTO_MEDIO_6M,IND_ERRO,IND_ERRO_6M,INTENCIONALIDADE,INTENCIONALIDADE_MEDIA_6M,INTENCIONALIDADE_DESVIO_6M)         
...
DROP TABLE IF EXISTS ST_ALERT_MONEYPASS_BMF_02_PADRAO