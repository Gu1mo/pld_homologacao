CREATE PROCEDURE [dbo].[FIRA_PR_CARGA_ALERT_301_MONEYPASS_CORRETORA] @PREGAO SMALLDATETIME, @AUX INT
--WITH ENCRYPTION	
AS
...
***********************************/
IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[ST_ALERT_MONEYPASS_CORRETORA_PADRAO]') AND type in (N'U'))
CREATE TABLE [dbo].[ST_ALERT_MONEYPASS_CORRETORA_PADRAO](
	DATA DATE,                           
    CD_CLIENTE INT,
	CD_CONTRAPARTE INT,
...
EXEC dbo.usp_sync_table_schema_add_alter_2012
  --@src_schema='dbo', @src_table='ST_ALERT_MONEYPASS_CORRETORA_PADRAO',
  @schema_name='dbo', @base_table='ST_ALERT_MONEYPASS_CORRETORA',
  @execute=1,
...
*/
 ...
 DROP TABLE IF EXISTS #base_util;		
  SELECT 
        F.DT_NEGOCIO	   AS data,
        F.CD_CLIENTE	   AS cod_cliente,
        F.HH_NEGOCIO	   AS hr_do_negocio,
        F.NR_NEGOCIO	   AS nr_negocio,
        F.CD_PAPEL		   AS ativo,
        F.CD_NATOPE		   AS operacao,
        F.QT_MULTIPLICADOR AS qtd_negocios,
    ...
    WHERE F.DT_NEGOCIO BETWEEN @INICIO AND @FIM;
...
GROUP BY i.cod_cliente, i.cod_contraparte, u.ativo;
 ...
 ------------------------------------------------------------6 MESES  INICIO
  ...
  SELECT 
	...
	INTO #base_util_6M
    FROM ST_CORRETAGEM_ORDEM F
    WHERE F.DT_NEGOCIO BETWEEN @INICIO_6M AND @FIM_6M;
...
GROUP BY i.cod_cliente, i.cod_contraparte, u.ativo,i.cd_anomes;
 ...
 DROP TABLE IF EXISTS #MoneyPass_6M_metrics_B;
  ...
  DROP TABLE IF EXISTS #MoneyPass_6M_metrics			   
    ...
    DELETE FROM ST_ALERT_MONEYPASS_CORRETORA WHERE DATA = CAST(@PREGAO-@AUX AS DATE)            
             
    INSERT INTO ST_ALERT_MONEYPASS_CORRETORA (DATA,CD_CLIENTE,CD_CONTRAPARTE,NR_NEGOCIOS,PAPEIS,QTD_TOTAL,RESULTADO,RESULTADO_MEDIA_6M,RESULTADO_DESVIO_6M,CONCENTRACAO,CONCENTRACAO_MEDIA_6M,CONCENTRACAO_DESVIO_6M,IND_ACERTO,IND_ACERTO_MEDIO_6M,IND_ERRO,IND_ERRO_6M,INTENCIONALIDADE,INTENCIONALIDADE_MEDIA_6M,INTENCIONALIDADE_DESVIO_6M)         
...
IF COL_LENGTH(N'dbo.ST_ALERT_MONEYPASS_CORRETORA', 'DT_FIRA') IS NOT NULL
BEGIN
    UPDATE [dbo].ST_ALERT_MONEYPASS_CORRETORA
...
DROP TABLE IF EXISTS ST_ALERT_MONEYPASS_CORRETORA_PADRAO