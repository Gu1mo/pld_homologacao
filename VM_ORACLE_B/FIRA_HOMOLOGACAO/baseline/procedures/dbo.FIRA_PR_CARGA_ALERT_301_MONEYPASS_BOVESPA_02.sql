CREATE PROCEDURE [dbo].[FIRA_PR_CARGA_ALERT_301_MONEYPASS_BOVESPA_02] @PREGAO SMALLDATETIME, @AUX INT
--WITH ENCRYPTION	
AS
...
***********************************/
IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[ST_ALERT_MONEYPASS_02_PADRAO]') AND type in (N'U'))
CREATE TABLE [dbo].[ST_ALERT_MONEYPASS_02_PADRAO](
	[DATA] [date] NULL,
	[CD_CLIENTE] [int] NULL,
...
EXEC dbo.usp_sync_table_schema_add_alter_2012
  --@src_schema='dbo', @src_table='ST_ALERT_MONEYPASS_02_PADRAO',
  @schema_name='dbo', @base_table='ST_ALERT_MONEYPASS_02',
  @execute=1,
...
SELECT
    ...
    CASE
        ...
        ELSE SUM(CAST(preco AS decimal(38,10)) * CAST(qtd_negocios AS decimal(38,10)))
...
GROUP BY i.cod_cliente, i.cod_contraparte, u.ativo;
 ...
 DROP TABLE IF EXISTS #DIRETAS_6M;
		SELECT DT_PERIODO
		  ...
		  INTO #DIRETAS_6M
		  FROM ST_DIRETAS_BVSP 
		 WHERE DT_PERIODO BETWEEN @INICIO_6M AND @FIM_6M
		   AND CD_CLIENTE_COMPRA <> CD_CLIENTE_VENDA;
 ...
 DROP TABLE IF EXISTS #base_util_6M;		 
		SELECT DT_PERIODO	      AS data
			 ...
			 , VL_TOTNEG_COMPRA   AS volume
			 , CD_CLIENTE_VENDA   AS cod_contraparte
			 , CONVERT(CHAR(6),DT_PERIODO, 112) AS cd_anomes
		  INTO #base_util_6M 
		  FROM #DIRETAS_6M
...
GROUP BY i.cod_cliente, i.cod_contraparte, u.ativo,i.cd_anomes;
 ...
 DROP TABLE IF EXISTS #MoneyPass_6M_metrics_B;
  ...
  DROP TABLE IF EXISTS #MoneyPass_6M_metrics			   
    ...
    DELETE FROM ST_ALERT_MONEYPASS_02 WHERE DATA = CAST(@PREGAO-@AUX AS DATE)            
             
    INSERT INTO ST_ALERT_MONEYPASS_02 (DATA,CD_CLIENTE,CD_CONTRAPARTE,NR_NEGOCIOS,PAPEIS,QTD_TOTAL,RESULTADO,RESULTADO_MEDIA_6M,RESULTADO_DESVIO_6M,CONCENTRACAO,CONCENTRACAO_MEDIA_6M,CONCENTRACAO_DESVIO_6M,IND_ACERTO,IND_ACERTO_MEDIO_6M,IND_ERRO,IND_ERRO_6M,INTENCIONALIDADE,INTENCIONALIDADE_MEDIA_6M,INTENCIONALIDADE_DESVIO_6M)         
...
IF COL_LENGTH(N'dbo.ST_ALERT_MONEYPASS_02', 'DT_FIRA') IS NOT NULL
BEGIN
    UPDATE [dbo].ST_ALERT_MONEYPASS_02
...
DROP TABLE IF EXISTS ST_ALERT_MONEYPASS_02_PADRAO