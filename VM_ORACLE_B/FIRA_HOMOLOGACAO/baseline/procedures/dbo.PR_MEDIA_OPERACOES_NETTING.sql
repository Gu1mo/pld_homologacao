CREATE PROCEDURE [dbo].[PR_MEDIA_OPERACOES_NETTING]  @ANOMES INT, @CD_CLIENTE VARCHAR (60), @QTDPORPAGINA INT, @PAGINA INT	
WITH RECOMPILE
AS		
...
;with t1 as (
	...
	FROM(
		SELECT 
			DT_NEGOCIO as data,
			NC.CD_CLIENTE,
			SUM(CASE WHEN NC.CD_NATOPE = 'C' THEN (VL_TOTNEG) ELSE 0 END) AS COMPRA,
			SUM(CASE WHEN NC.CD_NATOPE = 'V' THEN (VL_TOTNEG) ELSE 0 END) AS VENDA			

		FROM ST_CORRETAGEM_ORDEM NC
...
format(pat.PATRIMONIO,'f','pt-br') AS [PATRIMONIO],
CASE WHEN ABS(t2.Total_MEs) > (pat.PATRIMONIO) THEN 'SIM' ELSE 'NAO' END [ALERTA MENSAL]
...
left outer join #PATRIMONIO pat on t1.[COD. DO CLIENTE] = pat.cd_cliente
