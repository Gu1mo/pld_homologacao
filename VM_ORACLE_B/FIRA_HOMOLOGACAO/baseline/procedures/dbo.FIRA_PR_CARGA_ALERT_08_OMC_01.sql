/****** Object:  StoredProcedure [dbo].[FIRA_PR_CARGA_ALERT_08_OMC_01]    Script Date: 25/02/2026 15:50:21 ******/

CREATE PROCEDURE [dbo].[FIRA_PR_CARGA_ALERT_08_OMC_01] @PREGAO SMALLDATETIME, @AUX INT
--WITH ENCRYPTION	
AS
...
***********************************/
IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[ST_ALERT_OMC_01_PADRAO]') AND type in (N'U'))
CREATE TABLE [dbo].[ST_ALERT_OMC_01_PADRAO](
	[DATA] [date] NULL,
	[CD_CLIENTE] [int] NULL,
	[NR_NEGOCIO] [int] NULL,
...
EXEC dbo.usp_sync_table_schema_add_alter_2012
  --@src_schema='dbo', @src_table='ST_ALERT_OMC_01_PADRAO',
  @schema_name='dbo', @base_table='ST_ALERT_OMC_01',
  @execute=1,
...
DELETE FROM ST_ALERT_OMC_01 WHERE DATA = CAST(@PREGAO-@AUX AS DATE) 	
	
INSERT INTO ST_ALERT_OMC_01	
(DATA,CD_CLIENTE,NR_NEGOCIO,QT_NEGOCIO_COMPRA,QT_NEGOCIO_VENDA,CD_PAPEL,HH_COMPRA,HH_VENDA,VL_TOTNEG_COMPRA,
VL_TOTNEG_VENDA,VL_NEGOCIO_COMPRA,VL_NEGOCIO_VENDA)
SELECT DISTINCT 
CAST(@PREGAO-@AUX AS DATE)  DATA
,CD_CLIENTE_COMPRA CD_CLIENTE
,NR_NEGOCIO,QT_NEGOCIO_COMPRA
,QT_NEGOCIO_VENDA
,CD_PAPEL,HH_COMPRA,HH_VENDA
,VL_TOTNEG_COMPRA
,VL_TOTNEG_VENDA
,VL_NEGOCIO_COMPRA
,VL_NEGOCIO_VENDA
FROM ST_DIRETAS_BVSP	
WHERE DT_PERIODO >= @dt_ini	
AND DT_PERIODO < @dt_Fim
AND CD_CLIENTE_COMPRA = CD_CLIENTE_VENDA	
GROUP BY 
CD_CLIENTE_COMPRA,NR_NEGOCIO,QT_NEGOCIO_COMPRA,
QT_NEGOCIO_VENDA,CD_PAPEL,HH_COMPRA,HH_VENDA,
VL_TOTNEG_COMPRA,VL_TOTNEG_VENDA,VL_NEGOCIO_COMPRA,
VL_NEGOCIO_VENDA
...
IF COL_LENGTH(N'dbo.ST_ALERT_OMC_01', 'DT_FIRA') IS NOT NULL
BEGIN
    UPDATE [dbo].ST_ALERT_OMC_01
...
DROP TABLE IF EXISTS ST_ALERT_OMC_01_PADRAO